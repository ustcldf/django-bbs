...
class Article(models.Model):
    forum = models.ForeignKey(Forum, verbose_name='版块')
    ...
    kw = jsonfield.JSONField(blank=True, null=True, default="{}")
...
class ArticleTag(models.Model):
    ...
    def post_save(cls, sender, instance, created, **kwargs):
        if created:
            ...
            for focus in Focus.get_focus_by_content_object(instance.tag).exclude(user=instance.article.user):
                notify.send(get_model(settings.AUTH_USER_MODEL).objects.get(id=settings.NOTIFICATION_USER),
                            recipient=focus.user,
                            verb=u'你关注的标签[{tag}]有新的文章'.format(tag=instance.tag.title),
                            action_object=instance,
                            description=instance.article.get_markdown()
                )

post_save.connect(ArticleTag.post_save, sender=ArticleTag)


@python_2_unicode_compatible
class Reply(models.Model):
    article = models.ForeignKey(Article, verbose_name='版块')
    created = CreationDateTimeField("创建的时间")
    body = models.TextField("文章内容")
    user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name='创建的用户')

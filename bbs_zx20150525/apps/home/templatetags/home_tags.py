        ...
        try:
            ...
            if content_type.app_label == 'article':
                return "<button type='button' {disabled} data-value='1' data-content_object='{resource_url}'" \
                       "class='btn btn-link open-comment {active1} vote_tag'><i class='fa fa-thumbs-up'></i> <span>{active1_count}</span></button>" \
                       "<button {disabled} type='button' data-value='0' data-content_object='{resource_url}' class='btn btn-link open-comment {active2} vote_tag'><i class='fa fa-thumbs-down'></i> <span>{active2_count}</span></button>".format(
                    resource_url=resource_url,
                ...
                return "<button type='button' {disabled} data-toggle='tooltip' data-content_object='{resource_url}' data-container='body' data-value='1' data-placement='bottom' title='这个答案是有用的' " \
                       ...
                       "data-placement='bottom' title='这个答案是没有用的' class='btn btn-link open-comment {active2} vote_tag'><i class='fa fa-thumbs-down'></i> <span>{active2_count}</span></button>".format(
                    resource_url=resource_url,
                    active1_count=avs.filter(agree=True).count(),
                    active2_count=avs.filter(agree=False).count(),
                    active1=agree == True and 'active' or '',

...
(function () {
    ...
    Markdown.Editor = function (markdownConverter, container, preview, help) {
        ...
        this.run = function () {
            ...
            if ($.cookie('data_editor_preview') == 'true') {
                $('.fa-eye', panels.buttonBar).parents('button').addClass('active');
                $(panels.preview).removeClass("hide").addClass("show");

            }else {
                $('.fa-eye', panels.buttonBar).parents('button').removeClass('active');
                $(panels.preview).addClass("hide").removeClass("show");
            }
        };
    ...
    function PanelCollection(container, preview) {
        this.buttonBar = container.find('#mde-button-bar')[0];
        this.preview = preview[0];
        this.input = container.find('.mde-input')[0];
    };
    ...
    ui.prompt = function (title, text, defaultInputText, callback) {
        ...
        var createDialog = function () {
            ...
            header.innerHTML = '<a class="close" data-dismiss="modal">×</a>  <h4 class="modal-title">'+title+'</h4>';
...
//
        ...
        function makeSpritedButtonRow() {
            ...
            group4 = makeGroup(4);
            buttons.quote = makeButton("wmd-quote-button", "引用 - " + ckeys + "+H", "fa fa-quote-left", bindCommand("doBlockquote"), group4);
...
//            buttons.video = makeButton("wmd-video-button", "视频 - " + ckeys + "+E", "fa fa-video-camera", bindCommand(function (chunk, postProcessing) {
...
//            }), group3);
            ...
            buttons.eye = makeButton("wmd-eye-button", "开关预览模式", "fa fa-eye", null, group6);
            buttons.eye.execute = function (manager) {
                ...
                {
                    $(panels.preview).addClass("show").removeClass("hide");
                    $.cookie('data_editor_preview', true);
                    $(this).addClass('active');
                }
            };
            buttons.help = makeButton("wmd-question-button", "编辑器语法帮助", "fa fa-question", null, group6);
            buttons.help.execute = function (manager) {
                if ($(buttonBar).find('.wmd-helper').is(':visible'))
                {
                    $(this).removeClass('active');
                    $(buttonBar).find('.wmd-helper').addClass("hide").removeClass("show");
                }
                else
                {
                    $(this).addClass('active');
                    $(buttonBar).find('.wmd-helper').addClass("show").removeClass("hide");
                }
            };
...
//                        '<pre>!![视频说明](视频地址) 例: !![优酷视频](http://youku.com)</pre>'+
            ...
            $('#mde-button-bar .wmd-helper ul li').click(function()
            {
                $(this).addClass('active').siblings().removeClass('active');
                $(this).parents('.wmd-helper').find('.content div').eq($(this).index()).addClass("show").removeClass("hide").siblings().addClass("hide").removeClass("show");
            });
        }
